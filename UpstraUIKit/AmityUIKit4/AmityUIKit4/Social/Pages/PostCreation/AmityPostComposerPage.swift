...
public struct AmityPostComposerPage: AmityPageView {
    ...
    public var body: some View {
        ...
        .onAppear {
            if AmityUIKitManagerInternal.shared.photoAssetByPhotosApp.count > 0
            {
                var medias: [AmityMedia] = []
                for image in AmityUIKitManagerInternal.shared
                    .photoAssetByPhotosApp
                {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    mediaAttatchmentViewModel.medias.append(contentsOf: medias)
                }
            } else {
                var medias: [AmityMedia] = []
                for url in AmityUIKitManagerInternal.shared
                    .videoAssetByPhotosApp
                {
                    let media = AmityMedia(
                        state: .localURL(url: url), type: .video)
                    media.localUrl = url
                    medias.append(media)
                }
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            ...
            Image(AmityIcon.getImageResource(named: closeIcon))
                ...
                .alert(isPresented: $showAlert) {
                    Alert(title: Text("Discard this post?"), message: Text("The post will be permanently deleted. It cannot be undone."), primaryButton: .cancel(Text("Keep editing")), secondaryButton: .destructive(Text("Discard"), action: {
                        AmityUIKitManagerInternal.shared
                            .photoAssetByPhotosApp.removeAll()
                        AmityUIKitManagerInternal.shared
                            .videoAssetByPhotosApp.removeAll()
