...
public struct AmityCreatePostMenuComponent: AmityComponentView {
    ...
    private let allowAllUserToCreateStory =
        AmityUIKitManagerInternal.shared.client.getSocialSettings()?.story?
    ...
    public init(isPresented: Binding<Bool>? = nil, pageId: PageId? = nil) {
        self._viewConfig = StateObject(
            wrappedValue: AmityViewConfigController(
                pageId: pageId, componentId: .createPostMenu))
    ...
    public var body: some View {
        VStack {
            HStack {
                ...
                getMenuView()
                    .scaleEffect(
                        showPostCreationMenuScaleEffect ? 1.0 : 0.0,
                        anchor: .topTrailing)
            }
            Spacer()
        }
        ...
        VStack(spacing: 24) {
            ForEach(postTypes, id: \.rawValue) { type in
                ...
                case .post:
                    let createPostButton =
                        viewConfig.getConfig(
                            elementId: .createPostButton, key: "image",
                            of: String.self) ?? ""
                    let createPostTitle =
                        viewConfig.getConfig(
                            elementId: .createPostButton, key: "text",
                            of: String.self) ?? ""
                    getItemView(
                        image: AmityIcon.getImageResource(
                            named: createPostButton), title: createPostTitle
                    )
                    .onTapGesture {
                        goToPostCreation()
                    }
                    .accessibilityIdentifier(
                        AccessibilityID.Social.CreatePostMenu.createPostButton)
                case .story:
                    let createStoryButton =
                        viewConfig.getConfig(
                            elementId: .createStoryButton, key: "image",
                            of: String.self) ?? ""
                    let createStoryTitle =
                        viewConfig.getConfig(
                            elementId: .createStoryButton, key: "text",
                            of: String.self) ?? ""
                    getItemView(
                        image: AmityIcon.getImageResource(
                            named: createStoryButton), title: createStoryTitle
                    )
                    .onTapGesture {
                        goToStoryCreation()
                    }
                    ...
                    .accessibilityIdentifier(
                        AccessibilityID.Social.CreatePostMenu.createStoryButton)
                case .poll:
                    let icon = AmityIcon.createPollMenuIcon
                    getItemView(
                        image: icon.getImageResource(), title: type.rawValue,
                ...
                case .liveStream:
                    getItemView(
                        image: AmityIcon.getImageResource(
                            named: "icon_post_type_livestream"),
                        title: "Live Stream"
                    )
                    .onTapGesture {
                        goToLiveCreation()
                    ...
                    .accessibilityIdentifier(
                        AccessibilityID.Social.CreatePostMenu
                            .createLivestreamButton
                    )
                    .isHidden(!AmityUIKitManagerInternal.shared.isLiveStream)
                }
            }
        }
    ...
    @ViewBuilder
    private func getItemView(
        image: ImageResource, title: String,
    ...
    private func goToPostCreation() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {
            let context = AmityCreatePostMenuComponentBehavior.Context(
                component: self)
            AmityUIKitManagerInternal.shared.behavior
                .createPostMenuComponentBehavior?.goToSelectPostTargetPage(
                    context: context)
        }
    }
    ...
    private func goToStoryCreation() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {
            let context = AmityCreatePostMenuComponentBehavior.Context(
                component: self)
            AmityUIKitManagerInternal.shared.behavior
                .createPostMenuComponentBehavior?.goToSelectStoryTargetPage(
                    context: context)
        }
    }
    ...
    private func goToPollCreation() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {
            let context = AmityCreatePostMenuComponentBehavior.Context(
                component: self)
            AmityUIKitManagerInternal.shared.behavior
                .createPostMenuComponentBehavior?.goToSelectPollPostTargetPage(
                    context: context)
        }
    }
    
    private func goToLiveCreation() {
        withoutAnimation {
            isPresented.toggle()
        }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {
            ...
            AmityUIKitManagerInternal.shared.behavior.createPostMenuComponentBehavior?.goToLiveStreamTargetPage(context: context)
        }
    }
}
