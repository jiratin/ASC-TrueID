...
public struct AmityNewsFeedComponent: AmityComponentView {
    ...
    @State private var hideMyCommunity: Bool = true
    ...
    public var body: some View {
        ...
        .onReceive(
            postFeedViewModel.$shouldHideMyComunityComponent,
            perform: { value in
                hideMyCommunity = value
            }
        )
    }
    ...
    func getPostListView() -> some View {
        List {
            ...
            if !hideMyCommunity {
                VStack(spacing: 0) {
                    MyCommunityHeadeComponent(myCommunitys: postFeedViewModel.myCommunity)
            ...
            if (!dataSource.isHidenShelf && AmityUIKitManagerInternal.shared.urlAdvertisement != "") {
                ShelfHeader()
                    .listRowInsets(EdgeInsets())
                    .modifier(HiddenListSeparator())
            }
