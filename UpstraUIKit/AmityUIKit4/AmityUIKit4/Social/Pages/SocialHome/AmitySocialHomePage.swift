...
class AmitySocialHomePageViewModel: ObservableObject {
    @Published var selectedTab: AmitySocialHomePageTab = .popularFeed
    ...
    init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(didTabButtonExploreCommunity(_:)), name: .didToExplorePage, object: nil)
    }
    ...
    @objc private func didPostCreated(_ notification: Notification) {
        selectedTab = .popularFeed
    }
    
    @objc private func didTabButtonExploreCommunity(_ notification: Notification) {
        selectedTab = .explore
    }
}
