...
class PostFeedViewModel: ObservableObject {
    ...
    @Published var shouldHideMyComunityComponent: Bool = true
    private let communityManager = CommunityManager()
    private var tokenMyCommunity: AmityNotificationToken?
    private var communityRepository: AmityCommunityRepository?
    var myCommunity: [AmityCommunityModel] = []
    ...
    func loadFeed(feedType: FeedType) {
        ...
        case .community(let communityId):
            ...
            prepareFeedPost(collection: collection, paginatorCommunityId: paginatorCommunityId)
            loadMyCommunityFeed()
        case .globalFeed:
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
                ...
                self.loadMyCommunityFeed()
            }
        case .popularFeed:
            isLoadingNextPage = true
            fetchPopularPostListFromAPI { [weak self] result in
                guard let self else { return }
                switch result {
                case .success(let posts):
                    self.prepareFeedPost(collection: posts, paginatorCommunityId: nil)
                case .failure:
                    DispatchQueue.main.async {
                        self.feedLoadingStatus = .loaded
                        self.postItems = []
                    }
                }
            }
    ...
    func prepareFeedPost(collection: AmityCollection<AmityPost>, paginatorCommunityId: String?) {
    ...
    func setPosts(posts: AmityCollection<AmityPost>) {
        var allPosts: [PaginatedItem<AmityPostModel>] = []
        for index in (0..<posts.count()) {
            if let post = posts.object(at: index) {
                allPosts.append(PaginatedItem(id: post.postId, type: .content(AmityPostModel(post: post))))
            }
        }
        self.postItems = postItems + allPosts
    }
...
extension PostFeedViewModel {
    ...
    func fetchPopularPostListFromAPI(
        completion: @escaping (
            Result<AmityCollection<AmityPost>, RequestError>
        ) -> Void
    ) {
        let serviceRequest = RequestPopularFeedID()
        serviceRequest.request(page: page) { [weak self] result in
            guard let self else { return }
            switch result {
            case .success(let reponse):
                DispatchQueue.main.async {
                    self.token = self.feedManager.getPopularFeedPosts(postIDs: reponse.post).observeOnce({ postCollection, change, error in
                        self.isLoadingNextPage = false
                        if error != nil {
                            completion(.failure(RequestError(errorCode: 0, errorMessage: "Load post Error")))
                        } else {
                            completion(.success(postCollection))
                        }
...
extension PostFeedViewModel {
    ...
    func loadMyCommunityFeed() {
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            self.tokenMyCommunity = self.communityManager.searchCommunitites(keyword: "", filter: .userIsMember).observeOnce({ communityCollection, change, error in
                for index in 0..<communityCollection.count() {
                    if (self.myCommunity.count < 9) {
                        guard let object = communityCollection.object(at: index) else { continue }
                        let model = AmityCommunityModel(object: object)
                        self.myCommunity = self.myCommunity + [model]
                    }
                }
                if (self.myCommunity.count > 0) {
                    self.shouldHideMyComunityComponent = false
                }
                
                self.tokenMyCommunity?.invalidate()
            })
        }
    }
}
